...
async function getFeed(params) {
    ...
    WITH candidates AS (
      SELECT
        ...
        ROUND(
          ST_Distance(p.location, ST_MakePoint($2, $1)::geography) / 1000.0,
          2
    ...
    LIMIT $4
  `;
    const queryParams = [lat, lng, radiusMeters, limit + 1];
    if (category)
        queryParams.push(category);
    if (cursorScore !== null)
        queryParams.push(cursorScore);
...
async function logImpression(placeId) {
    // Increment in Redis sorted set — flushed to postgres by a background job
    await redis_1.default.hincrby('impressions:pending', placeId, 1);
}
